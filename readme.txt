=== Corpiva ===
Contributors: DesertThemes
Requires at least: 4.7
Tested up to: 6.8
Requires PHP: 5.6
Stable tag: 1.0.70
License: GPLv3 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.html
Tags: one-column, two-columns ,right-sidebar, flexible-header, custom-background, custom-header, custom-menu, editor-style, featured-images, footer-widgets, post-formats, theme-options, threaded-comments, translation-ready, full-width-template, custom-logo, blog, e-commerce, portfolio


== Description ==

Corpiva is lightweight, highly extendable and multi-purpose WordPress Theme. Corpiva is perfect for Startup, IT Solutions, IT Business, Agencies, Marketing, Consulting and Services Company related to any business websites. Corpiva based on in-depth research into IT Solutions, IT Services, IT Business, Consulting & Software, Digital Solution, Technical Engineering, Technology, and Services Company, to deliver all needed layouts and blocks. It will enable you to create almost any type of website with a beautiful & professional design. Corpiva supports popular WordPress plugins such as Elementor, WPML, Polylang, Yoast SEO, WooCommerce, Contact Form 7, Jetpack, and much more. Corpiva Pro demo https://preview.desertthemes.com/pro/corpiva/


== Installation ==
	
1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Corpiva in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.


== Copyright ==

Corpiva WordPress Theme, Copyright (c) 2024, Desert Themes
Corpiva is distributed under the terms of the GNU GPLs



== Credits ==

* Underscores - (C) 2012-2017 Automattic, Inc. - http://underscores.me/
License: GPLv2 or later (https://www.gnu.org/licenses/gpl-2.0.html) 

* Font Awesome 6.3.0 
Licenses - Font: SIL OFL 1.1, CSS: MIT License(http://fontawesome.io/license)
Source: https://github.com/FortAwesome/Font-Awesome

* Owl Carousel 2.3.4 - by @David Deutsch
Licenses - MIT License(https://github.com/OwlCarousel2/OwlCarousel2/blob/master/LICENSE)
Source: https://github.com/OwlCarousel2

* WOW - by Thomas Grainger
Licenses - MIT License (https://github.com/graingert/WOW/blob/master/LICENSE)
Source: https://wowjs.uk/

* Appear 
Licenses - MIT License (https://opensource.org/license/mit/)
Source: https://github.com/bas2k/jquery.appear/

* AOS
Licenses - MIT License (https://github.com/michalsnik/aos/blob/next/LICENSE)
Source: https://github.com/michalsnik/aos

* Odometer
Licenses - MIT License (https://github.com/HubSpot/odometer/blob/master/LICENSE)
Source: https://github.com/HubSpot/odometer

* Lenis 
Licenses - MIT License (https://github.com/darkroomengineering/lenis/blob/main/LICENSE)
Source: https://github.com/darkroomengineering/lenis

* ScrollTrigger 
Licenses - MIT License (https://github.com/terwanerik/ScrollTrigger/blob/master/LICENSE)
Source: https://github.com/terwanerik/ScrollTrigger

* SplitText 
Licenses - MIT License (https://github.com/netgfx/SplitText/blob/master/LICENSE.md)
Source: https://github.com/netgfx/SplitText

* Fancybox 
Licenses - GPLv3 License (https://github.com/fancyapps/fancybox)
Source: https://github.com/fancyapps/fancybox

* Animate Css by @Daniel Eden - http://daneden.me/animate
Licenses - MIT License(https://github.com/daneden/animate.css/blob/master/LICENSE)
Source: https://github.com/daneden/animate.css

* Customizer Controls 
Licenses - MIT license  (https://github.com/Codeinwp/customizer-controls/blob/master/LICENSE)
Source: https://github.com/Codeinwp/customizer-controls

* Customizer Repeater 
Licenses - MIT license (https://github.com/cristian-ungureanu/customizer-repeater/blob/production/LICENSE)
Source: https://github.com/cristian-ungureanu/customizer-repeater

* Range Controls 
Licenses - MIT license  (https://github.com/Codeinwp/customizer-controls/blob/master/LICENSE)
Source: https://github.com/Codeinwp/customizer-controls/tree/master/customizer-range-control

* WP Bootstrap Navwalker
Licenses - GNU GENERAL PUBLIC LICENSE (https://github.com/wp-bootstrap/wp-bootstrap-navwalker/blob/master/LICENSE.txt)
Source: https://github.com/wp-bootstrap/wp-bootstrap-navwalker

* Sainitization
Licenses - GNU General Public License (https://github.com/WPTRT/code-examples/blob/master/LICENSE)
Source: https://github.com/WPTRT/code-examples

* Screenshot Banner Image - https://pxhere.com/en/photo/595817
License: Creative Commons Zero

* Page Title Image - https://pxhere.com/en/photo/1087339
License: Creative Commons Zero


== Changelog ==

@version 1.0.70
* Style improvements - Block Archives Color

@version 1.0.69
* Style improvements - Block Archives

@version 1.0.68
* Tested - By WordPress 6.8.2

@version 1.0.67
* Style improvements - Block Archives

@version 1.0.66
* Style improvements - Block Archives

@version 1.0.65
* Style improvements - Block Preformatted

@version 1.0.64
* Style improvements - Block Verse

@version 1.0.63
* Style improvements - Block Details

@version 1.0.62
* Style improvements - Block List

@version 1.0.61
* Style improvements - Block List

@version 1.0.60
* Style improvements - Block Page List Hover & Focus

@version 1.0.59
* Style improvements - Block Page List

@version 1.0.58
* Style improvements - Block Page List

@version 1.0.57
* Style improvements - Block Page List

@version 1.0.56
* Style improvements

@version 1.0.55
* Tested - By WordPress 6.8

@version 1.0.54
* Style improvements - Block Page List

@version 1.0.53
* Style improvements - Block Avatar

@version 1.0.52
* Style improvements - Block Avatar

@version 1.0.51
* Style improvements - Block Comment Date

@version 1.0.50
* Style improvements - Block Comment Author Name

@version 1.0.49
* Style improvements - WooCommerce Customer Account Icon

@version 1.0.48
* Style improvements - WooCommerce Cart Page Remove

@version 1.0.47
* Style improvements - WooCommerce Cart Page Coupon

@version 1.0.46
* Tested - By WordPress 6.7.2

@version 1.0.45
* Style improvements - WooCommerce Block Cart Badge

@version 1.0.44
* Style improvements - Block Comments

@version 1.0.43
* Style improvements - WooCommerce Block Cart

@version 1.0.42
* Style improvements - WooCommerce Block Cart Description

@version 1.0.41
* Style improvements - Block Archives

@version 1.0.40
* Style improvements - WooCommerce Block Cart Total Label

@version 1.0.39
* Style improvements - WooCommerce Block Cart Button

@version 1.0.38
* Style improvements - WooCommerce Block Cart Title

@version 1.0.37
* Style improvements - WooCommerce Block Cart Badge

@version 1.0.36
* Style improvements

@version 1.0.35
* Fixed - Blog View Issue

@version 1.0.34
* Style improvements - WooCommerce Block Cart Button

@version 1.0.33
* Style improvements - Block Verse

@version 1.0.32
* Style improvements - WooCommerce Block Cart Checkout Button

@version 1.0.31
* Style improvements - Block Details

@version 1.0.30
* Style improvements - WooCommerce Block Cart

@version 1.0.29
* Tested - By WordPress 6.7.1

@version 1.0.28
* Style improvements - Block Details

@version 1.0.27
* Tested - By WordPress 6.7

@version 1.0.26
* Updated - WooCommerce Template Files

@version 1.0.25
* Style improvements - WooCommerce Block Cart

@version 1.0.24
* Style improvements - Block Post Author

@version 1.0.23
* Style improvements - WooCommerce Block Product

@version 1.0.22
* Style improvements - WooCommerce Block Cart

@version 1.0.21
* Style improvements - Block Page List

@version 1.0.20
* Style improvements - WooCommerce Block Cart Button

@version 1.0.19
* Style improvements - Block Author Image

@version 1.0.18
* Style improvements - WooCommerce Block Cart Product Price

@version 1.0.17
* Style improvements - Block Mini Cart

@version 1.0.16
* Style improvements - WooCommerce Block Cart Product Description

@version 1.0.15
* Style improvements - WooCommerce Block Cart Product Title

@version 1.0.14
* Style improvements - Block Code Background Color

@version 1.0.13
* Style improvements - WooCommerce Handpicked Product Button

@version 1.0.12
* Tested - By WordPress 6.6.2

@version 1.0.11
* Style improvements - Block Code Text Color

@version 1.0.10
* Style improvements - WooCommerce Handpicked Product Button Hover & Focus

@version 1.0.9
* Style improvements - Block Table

@version 1.0.8
* Style improvements - WooCommerce Handpicked Product Button

@version 1.0.7
* Style improvements - Block Table

@version 1.0.6
* Style improvements - WooCommerce Featured Product Button

@version 1.0.5
* Fixed - Block Quote Color Issue

@version 1.0.4
* Style improvements - WooCommerce Featured Product Price

@version 1.0.3
* Fixed - Block Text Color Issue

@version 1.0.2
* Style improvements - WooCommerce Store Breadcrumbs

@version 1.0.1
* Style improvements - WooCommerce Featured Category Button

@version 1.0
* Style improvements - Block Read More

@version 0.9
* Style improvements - Block Video Caption

@version 0.8
* Style improvements - WooCommerce Checkout Page Summary

@version 0.7
* Style improvements - Block Image Caption

@version 0.6
* Tested - By WordPress 6.6.1

@version 0.5
* Style improvements - WooCommerce Cart Page

@version 0.4
* Style improvements - WooCommerce Checkout Page Button     

@version 0.3
* Style improvements - WooCommerce Cart Page Product Badge  

@version 0.2
* Added Theme URI
* Added Upsale Link in Customizer

@version 0.1
* Initial release